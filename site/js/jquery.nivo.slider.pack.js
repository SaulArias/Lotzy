...
$('a.nivo-prevNav',slider).live('click',function(){if(vars.running)return false;k=1;clearInterval(timer); timer='';vars.currentSlide-=2;nivoRun(slider,kids,settings,'prev');});$('a.nivo-nextNav',slider).live('click',function(){if(vars.running)return false;k=-1;clearInterval(timer);timer='';nivoRun(slider,kids,settings,'next');});}
...
if(vars.currentImage.attr('title')!=''){var title=vars.currentImage.attr('title');if(title.substr(0,1)=='#')title=$(title).html();if($('.nivo-caption',slider).css('display')=='block'){
																						  now=now+k*w1;k=-1;
		pos=now+'px 95px';
		$('.bg_slider').animate({backgroundPosition:pos},400);	$('.nivo-caption',slider).animate({bottom:'-240'},200).fadeOut(settings.animSpeed,function(){$(this).html(title);$(this).animate({bottom:'10'},280).fadeIn(settings.animSpeed);});}else{$('.nivo-caption',slider).html(title);}
